...
namespace MyNamespace
{
	...
	{
		// percolate hole to bottom, then push val, using pred
		// precondition: bottom != 0
		const ptrdiff_t top = hole;
		ptrdiff_t idx = hole;

		while (1) { 
			if (hole * 2 + 1 >= bottom)break;
			else if (hole * 2 + 2 >= bottom) { first[hole] = first[hole * 2 + 1]; hole = hole * 2 + 1; }
			else { 
				
				if (pred(first[hole * 2 + 2], first[hole * 2 + 1]))
					idx = hole * 2 + 1, first[hole] = first[idx], hole = hole * 2 + 1;
				else
					idx = hole * 2 + 2, first[hole] = first[idx], hole = hole * 2 + 2;
